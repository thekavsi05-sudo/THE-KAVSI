...
export default function ProductDetails() {
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-24 text-center text-stone">
        Loading…
      </div>
    );
  }
  if (!product) {
    return (
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-24 text-center">
        ...
        <Link to="/shop" className="btn-ghost mt-3 inline-flex">
          Back to Shop
        </Link>
      </div>
    );
  ...
  const variantSoldOut =
    variantChosen && Boolean(variant) && variantStock === 0;
  ...
  async function handleAddToCart() {
    ...
    try {
      ...
      if (!available) {
        toast.error(
          stock === 0
            ? `${color} / ${size} just sold out`
            : `Only ${stock} left in ${color} / ${size}`,
        );
        return;
      }
      ...
      <div className="grid md:grid-cols-2 gap-10 lg:gap-16">
        ...
        <div>
              ...
              {product.sizes.map((s) => {
                ...
                return (
                  <button
                    ...
                    title={
                      !available ? `${s} unavailable in ${color}` : undefined
                    }
          ...
          <div className="mt-6">
            ...
            <p
              className={`text-xs mt-2 flex items-center gap-1.5 ${variantSoldOut ? "text-wine" : getStockStatus(variantStock, product.lowStockThreshold) === "low" ? "text-wine-light" : "text-stone"}`}
            >
              {(variantSoldOut || variantOutOfStock) && (
                <AlertCircle size={13} />
              )}
              {!variantChosen
          ...
          {productOutOfStock && (
            <p className="text-xs text-wine mt-2 text-center">
              This product is currently out of stock in all sizes and colors.
            </p>
          )}

          <div className="grid grid-cols-2 gap-3 mt-6 text-xs text-stone">
            <div className="flex items-center gap-2">
              <Truck size={16} className="text-wine" /> Delivered in 3–5 days
            </div>
            <div className="flex items-center gap-2">
              <ShieldCheck size={16} className="text-wine" /> Quality checked
            </div>
          </div>
        </div>
      </div>
